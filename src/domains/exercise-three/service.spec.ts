...
describe('ExerciseTwoService unit tests', () => {
    ...
    it('Should return that the word is happy but is neither prime nor multiple of 3 or 5', () => {
        expect(service.validadeExpression('W'))
            .toStrictEqual({ isHappy: true, isPrime: false, isMultipleFiveOrThree: false });
    });

    it('Should return that the word is happy and prime but not multiple of 3 or 5', () => {
        expect(service.validadeExpression('abca'))
            .toStrictEqual({ isHappy: true, isPrime: true, isMultipleFiveOrThree: false });
    });

    it('Should return that the word is happy and multiple of 3 or 5 but not prime', () => {
        expect(service.validadeExpression('abcd'))
            .toStrictEqual({ isHappy: true, isPrime: false, isMultipleFiveOrThree: true });
    });

    it('Should return that the word is happy and multiple of 3 or 5 but not prime', () => {
        expect(service.validadeExpression('11111'))
            .toStrictEqual({ isHappy: false, isPrime: false, isMultipleFiveOrThree: true });
    });
