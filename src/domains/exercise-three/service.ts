import ExerciseOneService from '~/domains/exercise-one/service';
import ExerciseTwoService from '~/domains/exercise-two/service';
...
class ExerciseThreeoService {
    ...
    constructor() {
        ...
        alphabet.forEach((letter, index) => (this.letterValues.set(letter, index + 1)));
        alphabet.forEach((letter, index) => (this.letterValues.set(letter.toUpperCase(), index + 27)));
    }
    ...
    validadeExpression(expression: string): ResponseExpressionValidate {
        ...
        const isMultipleFiveOrThree = exerciseOneService.isMultipleOfFive(valueOfExpression)
